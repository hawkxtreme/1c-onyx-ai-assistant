...
Функция ПолучитьТекстОтветаOnyx(ОтветOnyx)
	...
	Если ТипЗнч(ОтветOnyx) = Тип("Структура") ИЛИ ТипЗнч(ОтветOnyx) = Тип("Соответствие") Тогда
		
		// Новый формат с полем answer
		ЗначениеAnswer = ПолучитьЗначениеИзДанных(ОтветOnyx, "answer");
		Если ЗначениеЗаполнено(ЗначениеAnswer) Тогда
			Возврат Строка(ЗначениеAnswer);
		КонецЕсли;
		
		// Старый формат (для обратной совместимости)
		МассивСообщений = ПолучитьЗначениеИзДанных(ОтветOnyx, "messages");
		Если ТипЗнч(МассивСообщений) = Тип("Массив") И МассивСообщений.Количество() > 0 Тогда
			ПервоеСообщение = МассивСообщений[0];
			Если ТипЗнч(ПервоеСообщение) = Тип("Структура") ИЛИ ТипЗнч(ПервоеСообщение) = Тип("Соответствие") Тогда
				ЗначениеMessage = ПолучитьЗначениеИзДанных(ПервоеСообщение, "message");
				Если ЗначениеЗаполнено(ЗначениеMessage) Тогда
					Возврат Строка(ЗначениеMessage);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		
		ЗначениеMessage = ПолучитьЗначениеИзДанных(ОтветOnyx, "message");
		Если ЗначениеЗаполнено(ЗначениеMessage) Тогда
			Возврат Строка(ЗначениеMessage);
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ПолучитьЗначениеИзДанных(Данные, ИмяПоля)
