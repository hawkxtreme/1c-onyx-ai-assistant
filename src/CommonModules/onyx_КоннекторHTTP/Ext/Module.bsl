...
//   ПараметрыЗапроса - Структура или Соответствие - параметры, добавляемые в URL (часть после ?).
//     Ключи и значения задаются строками; для передачи нескольких значений одного параметра используйте массив строк.
...
//
// Возвращаемое значение:
//   Структура - ответ на выполненный запрос. Поля структуры:
//     ВремяВыполнения (Число, миллисекунды), Cookies (Соответствие), Заголовки (Соответствие),
//     ЭтоПостоянныйРедирект (Булево), ЭтоРедирект (Булево), Кодировка (Строка),
//     Тело (ДвоичныеДанные), КодСостояния (Число), URL (Строка).
//
...
// Параметры:
//   Ответ - См. НовыйОтвет
//   ПараметрыПреобразованияJSON - Структура - задает параметры преобразования JSON. Поддерживаемые поля:
//     ПрочитатьВСоответствие (Булево) - если Истина, чтение выполняется в Соответствие; иначе используется Структура.
//     ФорматДатыJSON (ФорматДатыJSON) - формат представления дат в строках.
//     ИменаСвойствСоЗначениямиДата (Массив строк) - перечень свойств JSON, для которых требуется восстановление дат.
//     ИмяФункцииВосстановления (Строка) - имя функции восстановления; функция принимает параметры
//       Свойство (Строка), Значение (Произвольный) и ДополнительныеПараметры (Произвольный) и должна вернуть преобразованное значение.
//     МодульФункцииВосстановления (Произвольный) - модуль, содержащий функцию восстановления.
//     ДополнительныеПараметрыФункцииВосстановления (Произвольный) - параметры, передаваемые функции восстановления.
//     ИменаСвойствДляОбработкиВосстановления (Массив) - список свойств, для которых вызывается функция восстановления.
//     МаксимальнаяВложенность (Число) - максимальный уровень вложенности JSON-объекта.
//
// Возвращаемое значение:
//   Соответствие - ответ сервера в виде десериализованного значения JSON (вариант по умолчанию).
//   Структура - используется, если ПараметрыПреобразованияJSON.ПрочитатьВСоответствие = Ложь.
//
...
//
// Возвращаемое значение:
//   Структура - структура URL. Включает поля:
//     Схема (Строка) - схема обращения к серверу (http, https).
//     Аутентификация (Структура) - параметры аутентификации с полями Пользователь и Пароль.
//     Сервер (Строка) - адрес сервера.
//     Порт (Число) - порт сервера.
//     Путь (Строка) - адрес ресурса на сервере.
//     ПараметрыЗапроса (Соответствие) - параметры запроса в части URL после "?"; значения задаются строками,
//       для множественных значений используйте массив строк.
//     Фрагмент (Строка) - часть URL после символа "#".
//
...
//   ПараметрыПреобразования - Структура - параметры сериализации JSON (по умолчанию используется кодировка utf-8).
//     Доступные поля: ФорматДатыJSON (ФорматДатыJSON), ВариантЗаписиДатыJSON (ВариантЗаписиДатыJSON),
//     ИмяФункцииПреобразования (Строка) и МодульФункцииПреобразования (Произвольный) для обработки нестандартных типов,
//     ДополнительныеПараметрыФункцииПреобразования (Произвольный) - параметры, передаваемые функции преобразования.
//     Функция преобразования должна быть экспортируемой и принимать аргументы Свойство (Строка),
//     Значение (Произвольный), ДополнительныеПараметры (Произвольный) и Отказ (Булево); возвращаемое значение - результат преобразования.
//   ПараметрыЗаписи - Структура - настройки записи JSON (ПереносСтрок (ПереносСтрокJSON), СимволыОтступа (Строка),
//     ИспользоватьДвойныеКавычки (Булево), ЭкранированиеСимволов (ЭкранированиеСимволовJSON),
//     ЭкранироватьУгловыеСкобки (Булево), ЭкранироватьРазделителиСтрок (Булево),
//     ЭкранироватьАмперсанд (Булево), ЭкранироватьОдинарныеКавычки (Булево), ЭкранироватьСлеш (Булево)).
//
// Возвращаемое значение:
...
//     ПрочитатьВСоответствие (Булево) - режим чтения (Соответствие или Структура).
//     ИменаСвойствСоЗначениямиДата (Массив строк) - свойства JSON, требующие восстановления дат из строк.
//     ФорматДатыJSON (ФорматДатыJSON) - формат десериализации дат.
//     ИмяФункцииВосстановления (Строка) - имя функции восстановления; функция принимает параметры
//     Свойство (Строка), Значение (Произвольный), ДополнительныеПараметры (Произвольный) и возвращает преобразованное значение.
//     МодульФункцииВосстановления (Произвольный) - модуль, содержащий функцию восстановления.
//     ДополнительныеПараметрыФункцииВосстановления (Произвольный) - дополнительные аргументы обработчика восстановления.
//     ИменаСвойствДляОбработкиВосстановления (Массив) - свойства, для которых вызывается обработчик восстановления.
//     МаксимальнаяВложенность (Число) - максимальная глубина JSON-объекта.
//
// Возвращаемое значение:
//   Произвольный - значение, десериализованное из JSON.
//
...
Функция ОтправитьЗапрос(Сессия, ПодготовленныйЗапрос, Настройки)
	...
	Если ОшибкаВыполненияЗапроса <> Неопределено Тогда
		ВызватьИсключение(ПодробноеОписаниеОшибки(ОшибкаВыполненияЗапроса));
	КонецЕсли;
...
Функция ПодробноеОписаниеОшибки(Информация)
	
	Если Информация = Неопределено Тогда
		Возврат "";
	КонецЕсли;
	
	Если Информация.Свойство("Описание") Тогда
...
//   ТипыСвойств - Соответствие - типы свойств, которые нужно восстановить (ключ - имя свойства, значение - исходный тип).
...
#Иначе
	...
	Исключение
		...
		ТекстОшибки = ПодробноеОписаниеОшибки(ИнформацияОбОшибке());
	КонецПопытки;
	ЧтениеZip.Закрыть();
